...
const OrderSummary = ({ totalPrice, items }) => {
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      ...
      const checkoutResponse = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/carrinho/checkout`,
        {
          method: 'POST',
          headers: {
      ...
      router.push(`/checkout?pedidoId=${pedido.id}`);
    } catch (error) {
  ...
  return (
    <div className="w-full max-w-lg lg:max-w-[340px] bg-slate-50/30 border border-slate-200 text-slate-500 text-sm rounded-xl p-7">
      <h2 className="text-xl font-medium text-slate-600">Pagamento</h2>
      ...
      <div className="flex gap-2 items-center">
        <input
          ...
          disabled
        />
        <label htmlFor="Pix" className="cursor-pointer opacity-50">
          Pix (Em breve)
        </label>
      </div>
        ...
        {selectedAddress ? (
          ...
          <div>
            {addressList.length > 0 && (
              <input className="border border-slate-400 p-2 w-full my-3 outline-none rounded"></input>
            )}
            <button
              className="flex items-center gap-1 text-slate-600 mt-1"
              onClick={() => setShowAddressModal(true)}
            >
              Adicionar um endereço <PlusIcon size={18} />
            </button>
          </div>
        )}
      </div>
