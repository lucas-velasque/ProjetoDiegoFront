...
export default function CategoryList() {
  ...
  const [categories, setCategories] = useState<CategoriaLeilao[]>([]);
  ...
  const loadCategories = async () => {
    try {
      const response = await categoriaLeilaoApi.getAll({
        page,
        limit: itemsPerPage,
  ...
  const openEdit = (category: CategoriaLeilao) => {
    setEditing({ ...category });
    setShowModal(true);
  };
  ...
  const saveEdit = async () => {
    if (!editing) return;
    try {
      await categoriaLeilaoApi.update(editing.id, {
        nome: editing.nome,
        tipo: editing.tipo,
